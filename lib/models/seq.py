...
class NODE(nn.Module):
    def __init__(self, df=None, **kwargs):
        ...
        self.df = df
        self.nfe = 0
        self.A = self.df.dense1.weight
        self.eig = np.sort(np.abs(np.real(np.linalg.eig(self.A.detach().cpu().numpy())[0])))
        self.stiff = self.eig[-1]/self.eig[0]
        self.A.to(DEVICE)
        self.elem_t = None
    def forward(self, t, x):
        self.nfe += 1
...
class HeavyBallNODE(NODE):
    def __init__(self, df, actv_h=None, gamma_guess=-3.0, gamma_act='sigmoid', corr=-100, corrf=True):
        ...
        self.sp = nn.Softplus()
        self.A = self.df.dense1.weight.to('cpu')
        self.eig = np.linalg.eig(self.A.detach().cpu().numpy())[0]
        self.Beig = np.append((gamma_guess + np.sqrt(gamma_guess**2 + 4*self.eig))/2,  (gamma_guess - np.sqrt(gamma_guess**2 + 4*self.eig))/2)
        self.Beig = np.sort(np.abs(np.real(self.Beig)))
        self.stiff = self.Beig[-1]/self.Beig[0]
        self.A.to(DEVICE)
    ...
    def forward(self, t, x):
        ...
        self.A = self.df.dense1.weight
        self.get_gamma()
        self.Beig = np.append((self.g + np.sqrt(self.g **2 + 4*self.eig))/2,  (self.g  - np.sqrt(self.g **2 + 4*self.eig))/2)
        self.Beig = np.sort(np.abs(np.real(self.Beig)))
        self.stiff = self.Beig[-1]/self.Beig[0]
    ...
    def get_gamma(self):
        for name,param in self.named_parameters():
            if name == 'gamma.param':
                self.g = self.gammaact(param[0]).detach().cpu().numpy()
                break
